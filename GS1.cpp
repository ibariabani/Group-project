...
#include"GS1.h"
using namespace std;
 double U1[10000][10000];
void GS1(double& a,double& b,double& V0,double& delta,double& TitMax){
 ...
 for (t=0;t<=TitMax; t++){
   for (i=1;i<=XitMax;i++){ //i and j start iterating from the side of the box containing cylinder
	 ...
	 U1[i][j]=0;
	   ...
	   U1[i][j]=(1.0/4.0)*(U1[i+1][j]+U1[i-1][j]+U1[i][j+1]+U1[i][j-1]);  //if not use Jacobi method
