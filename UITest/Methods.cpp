...
/*
  ...
  log <<"The Jacobi Method took " << it <<" iterations to converge"<<endl;  //Outputs the number of iterations
  log.close();
...
void Methods::Gauss(double a, double b, double c){
  ...
  double ItMax=GS/delta;
  double x=0,y=0, Pre=0, Post=0,Err=0, ErrTotal=10000;
...
/*
    log.open("log.txt", ofstream::app);
    log<< "Gauss Siedell took "<<Git<<" iterations to converge"<< endl;  //Outputs
    log.close();
    file.open(QIODevice::ReadOnly | QIODevice::Text);
    ui->OutputBox->setText(in.readAll());
*/
 
  ofstream file1;               //Opens file to write output data to
  file1.open("vGS");
...
void Methods::SOR(double a, double b,double c){
  ...
  float Us[1000][1000];
  double resid=0,x=0,y=0, ErrTotal=10000, omega=0, Uold=0;
